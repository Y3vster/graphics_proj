...
vec2 p6m_fn() {
    ...
    for (int k = 0; k < 10; k++) {
        ...
        vec2 p4 = vec2(cos(         m[k]  * xhex() +  n[k]         * yhex()), 0);
        vec2 p5 = vec2(cos(  n[k]         * xhex() - (n[k] + m[k]) * yhex()), 0);
        vec2 p6 = vec2(cos(-(n[k] + m[k]) * xhex() +         m[k]  * yhex()), 0);
	    vec2 thisterm = (p1 + p2 + p3 + p4 + p5 + p6) / 6.0;
